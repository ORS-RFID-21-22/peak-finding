...
function findPeaksOld(arr, numPeaks, stdDevs) {
    var peaks = []
    var stats = stats(arr);
...
// arr = range profile array
// rangePairs = array of range window pairs as index ranges
//      EX:
// [
//     [[152, 174], [361, 383]],
//     [[100, 120], [400, 420]]
...
// ], 
// 
function findPeaksSeparateWindows(arr, rangePairs, stdDevs) {
    var peaks = [];
...
function findPeaksSameWindow(arr, numTags, rangePair) {
    ...
    if (imPeaks.length < numTags) return null;
    var rePeaks = findPeaksInRange(arr, numTags, rangePair[1][0], rangePair[1][1]);
    if (rePeaks.length < numTags) return null;
    ...
    function findPeaksInRange(arr, numPeaks, indexMin, indexMax) {
        var peaks = []
        var avg = arr.slice(indexMin, indexMax + 1).reduce((a, b) => a + b) / arr.length;
        var threshold = avg + 6;
        for (var i = indexMin; i <= indexMax; i++) {
            if (arr[i] > threshold && arr[i-1] < arr[i] && arr[i] > arr[i+1]) {
                if (peaks.length < numPeaks) {
...
document.addEventListener('DOMContentLoaded', function(e) {
    ...
    // console.log(peaks);
    for (var i = 0; i < rp_two_tags.length; i++) {
        plot(
            rp_two_tags[i], 
            findPeaksSameWindow(
                rp_two_tags[i], 
...
function plot(d, peaks, id) {
    ...
    for (var i = 0; i < peaks.length; i++) {
        ...
        for (var j = 0; j < peaks.length; j++) {
            var textbox = svg
            .append("text")
            .attr("x", 100).attr("y", 230 + 60*i+20*(j+1)).style("fill", colors[i]);
            textbox.text("index: " + peaks[i][j].x + " value: " + peaks[i][j].y);
        }
        // calculate range
        var det1 = rp_x[peaks[i][0].x];
        var det2 = rp_x[peaks[i][1].x] - rp_x[512];
        var range = (det1 + det2) / 2;

        var textbox = svg
        .append("text")
        .attr("x", 100).attr("y", 230 + 60*i+60).style("fill", colors[i]);
        textbox.text("range: " + range);
    }
}
